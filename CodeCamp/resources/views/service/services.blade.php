    ...
    <div class="p-6 lg:p-8">
        ...
        <div id="analysisContent" class="space-y-6">
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <p id="foodCategory" class="text-gray-700 text-base leading-relaxed">
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <p id="healthAssessment" class="text-gray-700 text-base leading-relaxed">
                    <!-- Will be populated by AI -->
                </p>
            </div>
            ...
            <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                ...
                <p id="recommendations" class="text-gray-700 text-base leading-relaxed">
                    <!-- Will be populated by AI -->
                </p>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let stream = null;
    let currentState = 'initial'; // initial, streaming, captured
    let facingMode = 'environment'; // environment (back camera) or user (front camera)
    ...
    if (!disclaimerAccepted) {
        ...
        // Hide disclaimer modal if already accepted
    ...
    // Decline Disclaimer - redirect to home or previous page
    declineDisclaimerBtn.addEventListener('click', function() {
        if (confirm('You must accept the disclaimer to use this service. You will be redirected to the home page.')) {
    ...
    closeDisclaimerBtn.addEventListener('click', function() {
        if (confirm('You must accept the disclaimer to use this service. Close anyway?')) {
            window.location.href = '/'; // Change to your home page route
        }
    });
    ...
    function updateButtonIcon(state) {
        const icon = cameraBtn.querySelector('svg');
        if (state === 'initial' || state === 'captured') {
            icon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />';
        } else if (state === 'streaming') {
            icon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path stroke-linecap="round" stroke-linejoin="round" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />';
        }
    }
    ...
    async function startCamera() {
        try {
            ...
            // Show rotate button only on mobile/tablet devices
            if (window.innerWidth <= 1024) {
                rotateCameraBtn.classList.remove('hidden');
            }
            
            cameraBtnText.textContent = 'Capture';
            updateButtonIcon('streaming');
    ...
    function captureImage() {
        ...
        cameraBtnText.textContent = 'Recapture';
        updateButtonIcon('captured');
    ...
    cameraBtn.addEventListener('click', function() {
        if (currentState === 'initial' || currentState === 'captured') {
            startCamera();
    ...
    analyzeBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            const blob = await new Promise(resolve => capturedImage.toBlob(resolve, 'image/jpeg'));
            ...
            document.getElementById('foodCategory').textContent = data.category || 'Unable to determine category';
            document.getElementById('healthAssessment').textContent = data.healthAssessment || 'No health assessment available';
            document.getElementById('recommendations').textContent = data.recommendations || 'No recommendations available';
            
            // Show recommendations, hide placeholder
